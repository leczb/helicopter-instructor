...
class TestHelicopterControl(unittest.TestCase):
    ...
    def test_wrap_180(self):
        ...
        self.assertAlmostEqual(wrap_180(180.0), -180.0)  # Modulo behavior: 180 % 360 - 180 = -180
    ...
    def test_pid_derivative_error(self):
        ...
        output1 = pid.update(error=2.0, dt=0.1) # first update, last_error initialized to 0.0
    ...
    def test_coordinate_transformer_facing_north(self):
        ...
        fwd, right = CoordinateTransformer.rotate_local_to_body_error(delta_x=10.0, delta_z=-5.0, heading_deg=0.0)
    ...
    def test_coordinate_transformer_facing_east(self):
        ...
        fwd, right = CoordinateTransformer.rotate_local_to_body_error(delta_x=10.0, delta_z=-5.0, heading_deg=90.0)
    ...
    def test_coordinate_transformer_velocity(self):
        ...
        fwd, right = CoordinateTransformer.rotate_local_to_body_velocity(vx=3.0, vz=4.0, heading_deg=0.0)
    ...
    def test_autopilot_controller_hover_signs(self):
        ...
        state = {
            'x': 0.0, 'y': 0.0, 'z': 2.0,  # 2m South
            'vx': 0.0, 'vy': 0.0, 'vz': 0.0,
            'phi': 0.0, 'theta': 0.0, 'psi': 0.0,
            'P': 0.0, 'Q': 0.0, 'R': 0.0
        }
        outputs = controller.update(dt=0.02, state=state)
        self.assertTrue(outputs['pitch'] < 0.0, f"Expected pitch command to be negative (nose down), got {outputs['pitch']}")
        ...
        state = {
            'x': -2.0, 'y': 0.0, 'z': 0.0,  # 2m West
            'vx': 0.0, 'vy': 0.0, 'vz': 0.0,
            'phi': 0.0, 'theta': 0.0, 'psi': 0.0,
            'P': 0.0, 'Q': 0.0, 'R': 0.0
        }
        outputs = controller.update(dt=0.02, state=state)
        self.assertTrue(outputs['roll'] > 0.0, f"Expected roll command to be positive (roll right), got {outputs['roll']}")
...
class TestAutopilotPlugin(unittest.TestCase):
    ...
    def test_flight_loop_callback_unpaused_normal_dt(self):
        ...
        self.plugin.controller.update = MagicMock(return_value={
            'roll': 0.1, 'pitch': 0.2, 'yaw': 0.3, 'collective': 0.5,
            'debug': {
                'fwd_err': 0.0, 'lat_err': 0.0, 'alt_err': 0.0, 'yaw_err': 0.0,
                'target_v_fwd': 0.0, 'target_v_lat': 0.0, 'target_v_vert': 0.0,
                'target_pitch': 0.0, 'target_roll': 0.0, 'v_fwd': 0.0,
                'v_lat': 0.0
            }
        })

        # Mock hardware scanner
        self.plugin.get_hardware_inputs = MagicMock(return_value={
    ...
    def test_get_gains_filepath_custom_aircraft(self):
        mock_xp.getNthAircraftModel.return_value = ("Bell_206.acf", "/path/to/Bell_206.acf")
    ...
    def test_get_gains_filepath_special_characters(self):
        mock_xp.getNthAircraftModel.return_value = ("Cessna-172SP (Custom).acf", "/path/to/Cessna-172SP (Custom).acf")
        filepath = config.get_gains_filepath(self.plugin.plugin_dir)
...
class TestHoverTargetAdjustment(unittest.TestCase):
    ...
    def test_reset_target_to_current(self):
        ...
        state = {
            'x': 12.0, 'y': 34.0, 'z': 56.0, 'psi': 78.0,
            'vx': 0.0, 'vy': 0.0, 'vz': 0.0,
            'phi': 0.0, 'theta': 0.0, 'P': 0.0, 'Q': 0.0, 'R': 0.0
        }
